 ...
 *   PROPOSAL  ??"ÂæÖÊ?Ê¥æÈ?Ê±? (open / amber) ??drag-sortable for HR prioritisation
 *   OFFICIAL  ??"Â∑≤Ê?Ê¥æÈ?Ê±? (assigned / green)
...
function DemandRow({ item, orgMembers, orgId }: DemandRowProps) {
  ...
  const handleAssign = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: '?íÁ?Â∑≤Ê?Ê¥?, description: `??{item.title}?çÊ??°Ê?Ê¥æÊ??ü„ÄÇ` });
        setSelectedMemberId('');
      } else {
        toast({ variant: 'destructive', title: '?áÊ¥æÂ§±Ê?', description: result.error.message });
      }
    } catch {
      toast({ variant: 'destructive', title: '?ç‰?Â§±Ê?', description: 'Ë´ãÁ?ÂæåÂ?Ë©¶„Ä? });
    } finally {
      setLoading(false);
    }
  ...
  const handleCancel = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: '?ÄÊ±ÇÂ∑≤?ñÊ?', description: `??{item.title}?çÂ∑≤??HR ?§Â??Ç` });
      } else {
        toast({ variant: 'destructive', title: '?ñÊ?Â§±Ê?', description: result.error.message });
      }
    } catch {
      toast({ variant: 'destructive', title: '?ç‰?Â§±Ê?', description: 'Ë´ãÁ?ÂæåÂ?Ë©¶„Ä? });
    } finally {
    ...
    <div className="space-y-3 rounded-lg border bg-background p-4">
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-1">
          ...
          {assignedMemberNames && (
            <p className="text-xs text-emerald-600">?áÊ¥æÁµ¶Ô?{assignedMemberNames}</p>
          )}
        </div>
        {statusBadge}
      </div>
        ...
        <div className="flex items-center gap-2">
          <Select value={selectedMemberId} onValueChange={setSelectedMemberId}>
            <SelectTrigger className="h-8 flex-1 text-xs">
              <SelectValue placeholder="?∏Ê??áÊ¥æ?êÂì°" />
            </SelectTrigger>
            <SelectContent>
              {orgMembers.map((m) => (
 ...
 *
 * "ÂæÖÊ?Ê¥æÈ?Ê±? cards are drag-sortable so HR can prioritise visually.
      ...
      <Card>
        <CardHeader className="border-b py-3">
          <CardTitle className="text-sm font-bold uppercase tracking-widest text-emerald-600">
            Â∑≤Ê?Ê¥æÈ?Ê±?({assignedItems.length})
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
