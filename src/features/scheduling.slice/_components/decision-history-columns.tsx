...
export const decisionHistoryColumns: ColumnDef<DecisionHistoryItem>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (status === "OFFICIAL") {
        return <Badge variant="secondary" className="border-green-500/20 bg-green-500/10 text-green-700"><CheckCircle className="mr-1 size-3"/>Â∑≤Ê†∏??/Badge>
      }
      if (status === "REJECTED") {
         return <Badge variant="destructive" className="border-red-500/20 bg-red-500/10 text-red-700"><XCircle className="mr-1 size-3"/>Â∑≤Ê?Áµ?/Badge>
      }
