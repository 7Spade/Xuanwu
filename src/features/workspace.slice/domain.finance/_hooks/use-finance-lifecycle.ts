  ...
  useEffect(() => {
    ...
    async function hydrateFinanceAggregate() {
      try {
        ...
        const parsingIntentRecords = (parsingIntents ?? []) as ParsingIntentRecord[];
        const hydratedItems = parsingIntentRecords
          .filter((intent: ParsingIntentRecord) => isActiveParsingIntentStatus(intent.status))
          .flatMap((intent: ParsingIntentRecord) =>
            intent.lineItems.flatMap((lineItem: ParsingIntentLineItem, index: number) => {
              const mapped = buildDirectiveItemFromParsingIntentLineItem(intent, lineItem, index);
              return mapped ? [mapped] : [];
            }),
          );

        const persistedDirectiveItems = (persistedState?.directiveItems ?? []) as FinanceDirectiveItem[];
        const persistedRemainingById = new Map(
          persistedDirectiveItems.map((item: FinanceDirectiveItem) => [item.id, item.remainingQuantity]),
        );

        const mergedDirectiveItems = hydratedItems.map((item: FinanceDirectiveItem) => ({
          ...item,
