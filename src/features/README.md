...
L0 External Triggers
  └─> L1 Command Boundary Gateway (CBG)
        └─> L2 Domain Slices  ← THIS LAYER (src/features)
              ...
              ├── global-search.slice
              ├── projection-bus (event projections → read-model views)
...
| `portal.slice` | — | Portal/dashboard shell state bridge |
| `finance.slice` | — | Finance domain (cost items, claims) |
...
| `infra.api-gateway` | — | API gateway adapter layer |
...
> **Current tracked D24 violations** (43 files — active migration targets, not new regressions): see `docs/architecture/README.md` for the full list.
...
| ❌ Forbidden | `src/features` imported by `src/shared`, `src/config`, or `src/shared-infra/firebase*` |
