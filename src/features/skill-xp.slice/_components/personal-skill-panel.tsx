...
export function PersonalSkillPanel() {
  ...
  if (loading) {
    return (
      <p className="py-12 text-center text-sm italic text-muted-foreground">è¼‰å…¥?€?½è??™â€?/p>
    );
  }
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">å°šç„¡?€?½ç???/p>
          <p className="mt-1 text-xs text-muted-foreground">
            å®Œæ??’ç?ä»»å?å¾Œç³»çµ±å??ªå?ç´¯ç? XP??
          </p>
        </CardContent>
      </Card>
    );
        ...
        <Card key={row.skillId} className="overflow-hidden">
          ...
          <CardContent className="space-y-2 pb-4 pt-0">
            ...
            <p className="text-[10px] text-muted-foreground">
              {row.xpInTier} / {row.xpNeeded} XP ??ä¸‹ä?ç­‰ç?
            </p>
          </CardContent>
        </Card>
      ))}
