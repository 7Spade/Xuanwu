...
export function OrgSettings() {
  ...
  useEffect(() => {
    if (activeAccount?.accountType === "organization") {
      ...
      setOrgDescription((activeAccount as { description?: string }).description ?? "");
    }
  }, [activeAccount]);
  ...
  const handleDelete = async () => {
    if (!confirm(t("settings.confirmDestroy").replace("{name}", activeAccount.name ?? ""))) return;
    setIsDeleting(true);
    try {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card className="border-border/60 shadow-sm">
        ...
        <CardFooter className="border-t bg-muted/20">
          <Button onClick={handleSave} disabled={isSaving} className="ml-auto text-xs font-bold uppercase tracking-widest">
            ...
            {isSaving ? t("common.creating") : t("settings.saveChanges")}
          </Button>
        </CardFooter>
      </Card>
      ...
      <Card className="border-2 border-destructive/30 bg-destructive/5 shadow-sm">
        ...
        <CardContent>
          <Button variant="destructive" onClick={handleDelete} disabled={isDeleting} className="text-xs font-bold uppercase tracking-widest">
