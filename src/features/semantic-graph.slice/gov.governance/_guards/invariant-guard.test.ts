...
import { addEdge, _clearEdgesForTest } from '../../domain.graph/_edges/_semantic-edge-store';

import { validateEdgeProposal } from './_invariant-guard';
...
describe('InvariantGuard ??IS_A cycle detection [D21-C D21-K]', () => {
  ...
  it('rejects a three-node transitive cycle (A??, then C?)', () => {
    addEdge('a', 'b', 'IS_A');
    addEdge('b', 'c', 'IS_A');
  ...
  it('approves a diamond-shaped IS_A graph (A?, B?) ??no cycle', () => {
    addEdge('a', 'c', 'IS_A');
