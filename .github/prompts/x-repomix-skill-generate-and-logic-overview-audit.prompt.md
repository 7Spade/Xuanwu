...
description: Generate Repomix context, audit repository architecture against logic-overview.md, and perform automatic corrections to align the codebase.
---
...
1. **Read Source of Truth:** Carefully read and absorb the core rules, invariants, and domain logic defined in `docs/architecture/logic-overview.md`.
...
3. **Contrast & Analyze:** Cross-reference the two documents and identify all gaps:
   ...
   - **Violations:** Improper imports, naming anomalies, or code acting outside of its designated layer as per `logic-overview.md`.
...
- **Source of Truth:** Treat `docs/architecture/logic-overview.md` as the absolute law. Any deviation in the current codebase is a bug that you MUST fix.
