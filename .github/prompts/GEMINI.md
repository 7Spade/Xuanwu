...
3. **文件為本**：任何決策必須對齊 `docs/` 資料夾下的 7 份核心文件，以 `00-LogicOverview.md` 為最高權威。
...
| `legacy-decoupling-specialist.prompt.md` | **遺留系統解耦專家**：設計分階段解耦策略、API 遷移計畫與兼容層，依「層級與依賴規則」「變動速率」「邊界與上下文」排序解耦優先序，控管停機風險。 | 需從單體或遺留模組分拆、逐步現代化時。 |
...
**⚠️ 注意：** 嚴禁繞過 `docs/00-LogicOverview.md` 進行任何非標準化修改。所有 UI 異動必須調用 `tool-shadcn`。