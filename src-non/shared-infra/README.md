...
src/shared/infra     (ACL Adapters — implements SK_PORTS)
  └──> src/shared-infra  ← THIS LAYER
         ├── firebase-admin/   (Cloud Functions, project config)
...
src/shared-infra/
├── README.md
├── firebase-admin/               # Server-side Firebase (Cloud Functions + project config)
...
Cloud Functions live in `firebase-admin/functions/` and are compiled as a **separate TypeScript package** from the Next.js app:

| Aspect | Detail |
|--------|--------|
| Package | `firebase-admin/functions/package.json` — independent `node_modules` |
| TypeScript | `firebase-admin/functions/tsconfig.json` — separate compilation |
| Root tsconfig exclusion | `tsconfig.json` excludes `src/shared-infra/firebase-admin/functions/**` |
| Lint | Not checked by root `npm run lint` / `npm run typecheck` |
| Deploy | `firebase deploy --only functions` |

> **[typecheck scope]:** `npm run typecheck` at the project root checks **only** the Next.js application code. The 67 TypeScript errors visible in `src/shared-infra/firebase-admin/functions/` are a separate concern and require `npm install --prefix firebase-admin/functions` to resolve in isolation.
...
**Design principle:** Security rules enforce data ownership and membership invariants that mirror the domain rules defined in `docs\architecture\00-logic-overview.md` — they are not the source of truth, but they must stay in sync with it.
...
| ✅ Allowed | `src/shared-infra/firebase-admin/functions` → `firebase-admin` (server SDK) |
...
src/shared/infra/firestore/firestore.facade.ts
  └──> imports Firebase Firestore instance
         └──> src/shared-infra/firebase-admin/  (or functions SDK)
```
...
| Functions type errors isolated from app typechecks | ✅ | Root `tsconfig.json` excludes `firebase-admin/functions/**` |
