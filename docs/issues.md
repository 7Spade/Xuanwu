...
> **Last verified**: 2026-03-03 — ARCH-D6-003 fixed (moved `_context.ts` to `_hooks/` per D6); `shared/types/skill.types.ts` converted to D19-compliant re-export stub  
> **Fixed in this cycle**: ARCH-D3-003 ✅, ARCH-D3-004 ✅, TYPE-D19-001 ✅, ARCH-D6-003 ✅  
...
> **Status**: 8 issues open  
> **Archive**: Resolved issues → [`docs/issues-archive.md`](./issues-archive.md) (9 entries)
...
|----------|----------|-------|-------|-------|
| ARCH (D6 Client Boundary) | 0 | 1 | 1 | 2 |
...
### ARCH-D6-003 ✅ RESOLVED

**Severity**: Minor (violates D6) — **Fixed 2026-03-03**  
**Rule**: D6 — `"use client"` 只在 `_components/` 或 `_hooks/` 葉節點

**Resolution**:  
The `"use client"` directive and React Context definition previously in `core.event-bus/_context.ts` (root-level placement, D6 violation) were already moved to `core.event-bus/_hooks/_context.ts` in a prior refactor. The import path `from "./_events"` inside `_hooks/_context.ts` was also corrected to `from "../_events"` so the module resolves correctly from within the `_hooks/` sub-directory.  

The root-level `_context.ts` now acts as a **legacy re-export barrel** (no `"use client"` directive) and `index.ts` continues to re-export from `_context.ts`, preserving all consumers unchanged.

**Archive**: → `docs/issues-archive.md`
...
| P11 | ARCH-D6-003 ✅ | `_context.ts` moved to `_hooks/_context.ts`; relative import corrected | D6 compliant — `"use client"` inside `_hooks/` |
