...
> **Last verified**: 2026-03-02 — 12 pre-existing issues re-confirmed; 4 new issues added (VS8/VS9/VS7 slices + workspace-provider.tsx)  
...
src/features/semantic-graph.slice/_types.ts
  → (uses TaxonomyNode, TaxonomyDimension, SemanticSearchHit, SearchDomain, SEARCH_DOMAINS)
...
src/features/semantic-graph.slice/_actions.ts
  → (uses types from '@/features/shared-kernel/semantic-primitives')
...
**Diagnosis**:  
`semantic-primitives` is a new sub-module added to `shared-kernel` as part of the VS8/VS9/VS7 expansion. It defines cross-BC contracts for taxonomy (`TaxonomyNode`, `TaxonomyDimension`), semantic search (`SearchDomain`, `SemanticSearchHit`, `SEARCH_DOMAINS`), and notification channel routing (`NotificationPriority`, `NotificationChannel`).

However, **none of these symbols are re-exported from `src/features/shared-kernel/index.ts`**. The `semantic-primitives` sub-directory is invisible to the shared-kernel public API. All 7 consuming files reach into the sub-path directly — a D7 violation that is architecturally equivalent to ARCH-D7-001.

Verification: `grep 'semantic-primitives' src/features/shared-kernel/index.ts` returns empty — the sub-module is completely absent from the shared-kernel barrel.

**Recommended Fix**:  
Add all public types from `semantic-primitives` to `src/features/shared-kernel/index.ts`:
```ts
// In shared-kernel/index.ts — Semantic Primitives (VS8/VS9/VS7) [D26]
export {
  SEARCH_DOMAINS,
...
```
Then update all 7 files to import from `@/features/shared-kernel` (root).
...
**Diagnosis**:  
Unlike ARCH-D7-004, both `buildIdempotencyKey` and `DlqTier` **are** already exported from `src/features/shared-kernel/index.ts` (lines 83–84). There is no need to reach into the sub-path. The bypass is redundant and creates an inconsistency with D7's index-only rule.

**Recommended Fix**:  
Change the import to use the shared-kernel root:
```ts
...
## ARCH Issues — Cross-Cutting Authority Violations (D26)
...
> ⚠️ **Removed from this table**: `workspace.slice/core/_components/workspace-provider.tsx` → `notification.slice`
> Previously marked ✅ (D7-compliant index path), but found to be **D26/A13 non-compliant** (uses legacy notification.slice routing instead of notification-hub.slice). Moved to ARCH-D26-001.
...
> Note: D5 violations (UI components importing `@/shared/infra/firestore`) are distinct from D24 violations. D24 checks for direct `firebase/*` SDK imports; D5 checks for infra-adapter imports in UI components. `workspace-provider.tsx` is D24-clean but D5-violating. See ARCH-D5-001.
...
|----------|----------|--------|--------|
| P0 | ARCH-D7-001 | 1-line index.ts export + 1-line import fix | Closes the only hard D7 sub-path bypass (missing export) |
| P1 | ARCH-D7-004 ⭐ | ~20 lines in shared-kernel/index.ts + 7 import fixes | Closes D7 gap for semantic-primitives (VS8/VS9/VS7 slices) |
| P2 | ARCH-D8-001 | Extract centralized-tag to dedicated slice | Fixes D8 purity; enables TAG_AUTH architectural separation |
| P3 | ARCH-D26-001 ⭐ | Replace `notification.slice` routing call in workspace-provider | Enforces notification-hub as sole A13 outlet; deprecates legacy router |
| P4 | ARCH-D5-001 ⭐ | Create `gov.audit/_actions.ts`; remove infra imports from workspace-provider | Fixes D5+D3 dual violation in UI component |
| P5 | ARCH-D3-001 | Refactor scheduling aggregate to return state, move writes to _actions.ts | Restores D3 compliance for scheduling mutations |
...
| P7 | ARCH-D6-001 | Amend D6 rule to allow _hooks/ client boundary OR wrap all hooks in _components/ | Resolves 28-file systematic D6 gap |
| P8 | TYPE-D20-001 | 1-line import change | Resolves D20 priority order in projection layer |
| P9 | TYPE-D19-001 | Move 3 type defs + update 1 file | Fixes ownership inversion for skill contracts |
| P10 | TYPE-D19-002 | Phase 1 only: promote `ScheduleItem`/`ScheduleStatus` | Highest-leverage cross-BC contract migration |
| P11 | ARCH-D7-005 ⭐ | 1-line import change in _outbox.ts | Redundant sub-path bypass; trivial fix |
| P12 | ARCH-D7-002/003 | 2-line change each page.tsx | Minor, but completes D7 compliance in `src/app/` |
| P13 | ARCH-D6-002 | Rename `_shell/` to `_components/shell/` or amend D6 | Low-risk rename |
| P14 | ARCH-D6-003 | Move `_context.ts` under `_components/` | 1-file relocation |
