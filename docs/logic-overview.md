...
%%    D21=VS8-neural-network-semantics(D21-1~D21-10 完整規範)
...
%%    [D21] VS8 四層語義神經網絡（Neural Network）：tag 類別在 VS8_CL 定義・TE1~TE6 強型別引用・禁止硬編碼語義
%%           Layer1=CL(神經元) → Layer2=SL(突觸) → Layer3=NG(計算) → Layer4=RL(輸出)
...
%% ─── 四層神經網路架構：
%% ───   Layer 1 VS8_CL — 神經元定義層  (Neuron Definition)   · 語義字典 · 定義權威 [D21-1 D21-2]
%% ───   Layer 2 VS8_SL — 語義突觸層    (Synapse Layer)        · IS_A / REQUIRES 加權邊圖 [D21-3 D21-9 D21-10]
%% ───   Layer 3 VS8_NG — 語義計算層    (Neural Computation)   · Dijkstra 前向傳播 + BFS 因果注意力 [D21-4 D21-6]
%% ───   Layer 4 VS8_RL — 語義決策輸出層 (Routing / Output)    · 成本語義路由 · 執行代理 [D21-5 D21-7]
%% ─── centralized-tag.aggregate 具備 lifecycle，為 domain authority [#A6 #17]
subgraph VS8["🧠 VS8 · Semantic Graph — The Brain [#A6 #17]（四層語義神經網路）"]
    direction TB

    subgraph VS8_CL["① 🏷️ 神經元定義層 (Neuron Definition Layer) — 語義字典・定義權威 [D21-1 D21-2]"]
        direction LR
    ...
    subgraph VS8_SL["② ⚡ 語義突觸層 (Synapse Layer) — 加權邊圖・網路拓撲 [D21-3 D21-9 D21-10]"]
        direction LR
        EDGE_STORE["centralized-edges/semantic-edge-store\n【突觸登錄中心 · 唯一邊圖操作點】\naddEdge(from, to, type, weight=1.0)\nremoveEdge(from, to, type)\ngetEdgesFrom(slug) / getEdgesTo(slug)\ngetEdgeWeight(from, to, type)\ngetEdgesByType(relationType)\n─────────────────────────────────────────\nIS_A (繼承/包含) — skill:expert IS_A skill:senior\nREQUIRES (依賴)  — role:lead REQUIRES skill:leadership\n─────────────────────────────────────────\n突觸強度 weight ∈ [0.0, 1.0]  (1.0 = 直接連結)\n語義代價 cost = 1.0 / max(weight, 0.001)  [D21-9]"]
        TOPO_OPS["拓撲運算 [D21-10]\nisSupersetOf(candidate, required)\n→ IS_A 傳遞閉包（VS6 排班資格）\ngetTransitiveRequirements(slug)\n→ REQUIRES 傳遞依賴（VS4 角色前置）"]
        EDGE_STORE -.->|"傳遞閉包查詢"| TOPO_OPS
    end

    subgraph VS8_NG["③ 🔗 語義計算層 (Neural Computation Layer) — 前向傳播 + 因果注意力 [D21-4 D21-6]"]
        direction LR
        NEURAL_NET["🧬 Neural Network [D21-3 D21-4]\ncomputeRelationWeight(from, to)\ncomputeSemanticDistance(a, b)\n→ { weightedDistance, hopCount }\ncomputeSemanticDistanceMatrix(slugs[])\n→ SemanticDistanceEntry[]\nfindIsolatedNodes(slugs[])\n→ D21-3 違規節點清單\n─────────────────────────────────────\nDijkstra 加權最短路徑\nedge cost = 1.0 / max(weight, MIN_EDGE_WEIGHT)\n強連結 = 短距離 (反向權重)"]
        CAUSALITY["🔍 Causality Tracer [D21-6]\ntraceAffectedNodes(event, candidates[])\nrankAffectedNodes(nodes[])\n→ (hopCount ASC, semanticWeight DESC)\nbuildDownstreamEvents(event, nodes[])\nbuildCausalityChain(event, candidates[])\n→ CausalityChain\n─────────────────────────────────────\nBFS 因果傳播（注意力機制）\nTAG_DELETED → 無下游事件\nTAG_DEPRECATED/STALE → 即時/延遲傳播"]
        ...
        NEURAL_NET -.->|"語義距離輸入 [D21-4]"| CAUSALITY
        CAUSALITY -->|"TagLifecycleEvent [D21-6]"| TAG_EV
        TAG_EV --> TAG_OB
        CAUSALITY -.->|"廢棄感知 [D21-8]"| TAG_SG
    end

    subgraph VS8_RL["④ 🚦 語義決策輸出層 (Routing / Output Layer) — 執行代理 [D21-5 D21-7]"]
        direction LR
    ...
    VS8_SL -->|"突觸拓撲輸入 · 讀取邊圖 [D21-3 D21-9]"| VS8_NG
...
%%  ╠══════════════════════════════════════════════════════════════════════════╣
%%  TAG SEMANTICS 擴展規則（VS8 · 四層語義神經網絡 [D21-1~D21-10]）
...
%%  ── 規則分層：Hard Invariants (D1~D20 核心不變量) / Semantic Governance D21(D21-1~D21-10)/D22~D23 / Infrastructure (D24~D25) / Authority Governance (D26) ──
%%  ── 基礎路徑約束（D1~D12）──
...
%%  ── 語義 Tag 守則（D21~D23）── VS8 四層語義神經網絡 (Neural Network) 正式規範 ──
