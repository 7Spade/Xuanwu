...
- `L6 Query Gateway`（`src/features/infra.gateway-query`）:
...
- `L8 Firebase Infrastructure`（Firebase runtime）:
	- 外部平台本體，不屬於本地業務層。
- `L9 Observability`（`src/features/observability`）:
	- 指標、錯誤、追蹤觀測；不承擔業務決策。
...
圖表命名規則：
- Mermaid 的 layer/slice/authority 標題應附上實際目錄路徑（例如 `src/features/infra.gateway-command`），避免責任邊界歧義。
