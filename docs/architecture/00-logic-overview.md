...
%%    寫鏈：shared-infra/external-triggers → shared-infra/gateway-command（L2 CMD_API_GW + CBG）→ *.slice → shared-infra/event-router → shared-infra/projection.bus
%%    讀鏈：app/UI → shared-infra/gateway-query（L6 QRY_API_GW + QGWAY）→ shared-infra/projection.bus
...
subgraph VS5["🟣 VS5 · Workspace Slice（src/features/workspace.slice）"]
    ...
    subgraph VS5_BIZ["⚙️ Business Domain（src/features/workspace.slice/domain.{tasks,quality-assurance,acceptance,daily,document-parser,files,issues,workflow} + domain.parsing-intent，A+B 雙軌）"]
        ...
        subgraph VS5_FIN["💰 Finance 事件橋接（src/features/workspace.slice → VS9 Finance Slice）[#A19 #A20]"]
            direction TB
