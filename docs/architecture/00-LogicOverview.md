...
%%      src/features/observability         = L9 Observability（非業務切片）
%%    命名規則：VS0=Foundation Index（L1+L7）；VS1~VS8=業務切片編號（L3）
%%    VS0 識別規格（文件/審查一律使用）:
...
%%    src/
%%      shared-kernel/                          # VS0-Kernel / L1: SK contracts/constants/pure functions only [D8]
...
%%  ── Scope（本輪必審）──
%%    1) VS0~VS8：每個編號域必須有明確層位與單一職責（VS0=L1+L7；VS1~VS8=L3）
