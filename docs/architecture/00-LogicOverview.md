...
%%    Path Map: VS0=src/shared-kernel
...
%%    ※ 邊界澄清：L1(Shared Kernel) 只承載契約；L6/L7/L8/L9 屬 Shared Infrastructure Plane（執行層）
...
%%      - 無 I/O（純型別/常數/純函式）→ src/shared-kernel/*（L1）
%%      - 有 I/O → 進 Step 2
%%    Step 2: 判斷「是否 Firebase SDK 呼叫」
%%      - 是 → src/shared-infra/frontend-firebase/*（L7 Adapter）
%%      - 否 → 進 Step 3
...
%%      - Observability                    → src/features/observability（L9）
%%    Step 5: 判斷「是否跨切片權威出口」
...
subgraph SK["🔷 L1 · Shared Kernel（VS0 · src/shared-kernel）— 契約/常數/純函式（No I/O）"]
    direction TB
...
subgraph SHARED_INFRA_PLANE["🧩 Shared Infrastructure Plane（Execution Plane：L6/L7/L8/L9，非 L1）"]
        direction TB
        ...
        subgraph FIREBASE_ACL["🔌 L7 · Firebase ACL Adapters（src/shared-infra/frontend-firebase）[D24 D25]"]
            direction LR
        ...
        subgraph OBS_LAYER["⬜ L9 · Observability（src/features/observability）"]
            direction LR
...
subgraph VS2["🟩 VS2 · Account Slice（src/features/account.slice）"]
    ...
    subgraph VS2_USER["👤 個人帳號域（src/features/account.slice/user.*）"]
        USER_AGG["user-account.aggregate"]
        WALLET_AGG["wallet.aggregate\n強一致帳本 [#A1]\n[S3: STRONG_READ]"]
        PROFILE["account.profile\nFCM Token（弱一致）"]
    end

    subgraph VS2_ORG["🏢 組織帳號域（src/features/account.slice/org.*）"]
        ORG_ACC["organization-account.aggregate"]
        ORG_SETT["org-account.settings"]
        ORG_BIND["org-account.binding\nACL 防腐對接 [#A2]"]
    end

    subgraph VS2_GOV["🛡️ 帳號治理域（src/features/account.slice/gov.*）"]
...
subgraph VS4["🟧 VS4 · Organization Slice（src/features/organization.slice）"]
    ...
    subgraph VS4_GOV["🛡️ 組織治理域（src/features/organization.slice/gov.*）"]
...
subgraph VS5["🟣 VS5 · Workspace Slice（src/features/workspace.slice）"]
    ...
    subgraph VS5_CORE["⚙️ Workspace Core Domain（src/features/workspace.slice/core.*）"]
    ...
    subgraph VS5_GOV["🛡️ Workspace Governance（src/features/workspace.slice/gov.*）"]
    ...
    subgraph VS5_BIZ["⚙️ Business Domain（src/features/workspace.slice/business.*，A+B 雙軌）"]
        direction TB
        ...
        subgraph VS5_A["🟢 A-track 主流程（src/features/workspace.slice/business.*）"]
            direction LR
        ...
        subgraph VS5_B["🔴 B-track 異常處理（src/features/workspace.slice/business.*）"]
            B_ISSUES{{"issues"}}
        end
