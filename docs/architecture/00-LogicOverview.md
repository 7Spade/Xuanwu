...
subgraph VS8["🧠 VS8 · Semantic Cognition Engine [#A6 #17]（四層語義引擎）"]
    direction TB

    subgraph VS8_GOV_LAYER["① 🏛️ Semantic Governance Layer"]
        direction TB
        ...
        subgraph VS8_GUARD["1.1 🛡️ guards · Semantic Integrity [D21-H D21-K S4]"]
            direction LR
            INV_GUARD["invariant-guard.ts\n【最高裁決權 · 語義衝突直接拒絕】\n違反物理邏輯聯結 → 攔截提案 [D21-H D21-K]"]
            STALE_MON["staleness-monitor.ts\nTAG_MAX_STALENESS ≤ 30s [S4 D21-8]"]
        end

        subgraph VS8_WIKI["1.2 🏛️ semantic-governance-portal [D21-I~W]"]
            direction LR
    ...
    subgraph VS8_CORE_LAYER["② 🧬 Semantic Core Domain"]
        direction TB

        subgraph VS8_CL["2.1 semantic-core-domain [D21-A D21-B D21-C D21-D]"]
            direction LR
            ...
            subgraph TAG_ENTS["🏷️ Semantic Tag Entities (TE1~TE6) [D21-A]"]
                direction LR
    ...
    subgraph VS8_ENGINE_LAYER["③ ⚙️ Semantic Compute Engine"]
        direction TB

        subgraph VS8_SL["3.1 graph-engine [D21-E D21-F D21-9 D21-10]"]
            direction LR
        ...
        subgraph VS8_NG["3.2 reasoning-engine [D21-4 D21-6 D21-X]"]
            direction LR
        ...
        subgraph VS8_ROUT["3.3 routing-engine [D21-5 D27-A]"]
            direction LR
        ...
        subgraph VS8_PLAST["3.4 learning-engine [D21-G]"]
            direction LR
    ...
    subgraph VS8_OUTPUT_LAYER["④ 📤 Semantic Output Layer"]
        direction TB

        subgraph VS8_PROJ["4.1 projections · 讀側投影 [D21-7 T5]"]
            direction LR
        ...
        subgraph VS8_IO["4.2 event-broadcast · 語義訂閱廣播 [D21-6 S1]"]
            direction LR
            LIFECYCLE_SUB["subscribers/lifecycle-subscriber.ts\n標籤生命週期事件訂閱"]
            TAG_OUTBOX["outbox/tag-outbox.ts\n[SK_OUTBOX: SAFE_AUTO]\n標籤異動廣播出口"]
        end

        subgraph VS8_RL["4.3 decision-policy · 語義決策輸出 [D21-5 D8 D27]"]
            direction LR
